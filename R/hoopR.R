...
tictoc::toc()
## 13.91 sec elapsed
glue::glue("NBA play-by-play data from {length(unique(nba_pbp$game_id))} games.")
## NBA play-by-play data from 4674 games.
dplyr::glimpse(nba_pbp)

### **NBA team box scores**------
tictoc::tic()
progressr::with_progress({
  nba_team_box <- hoopR::load_nba_team_box(2003:2021)
})
tictoc::toc()
glue::glue("NBA team boxscore data from {length(unique(nba_team_box$game_id))} games.")

### **NBA full player box scores**
tictoc::tic()
progressr::with_progress({
  nba_player_box <- hoopR::load_nba_player_box(2002:2021)
})
tictoc::toc()
glue::glue("NBA player boxscore data from {length(unique(nba_player_box$game_id))} games.")

###---- MBB Trimmed for length ---------- 
### **Men's college basketball full play-by-play seasons (2004-2021) ~ 45-90 seconds**
tictoc::tic()
progressr::with_progress({
  mbb_pbp <- hoopR::load_mbb_pbp(2006:2021)
})
tictoc::toc()
glue::glue("MBB play-by-play data from {length(unique(mbb_pbp$game_id))} games.")
dplyr::glimpse(mbb_pbp[1:53])
### **Men's college basketball full team box score seasons (2006-2021) ~ 5-30 seconds**
tictoc::tic()
progressr::with_progress({
  mbb_team_box <- hoopR::load_mbb_team_box(2006:2021)
})
tictoc::toc()
glue::glue("MBB team boxscore data from {length(unique(mbb_team_box$game_id))} games.")


### **Men's college basketball full player box score seasons (2006-2021) ~ 5-30 seconds**

tictoc::tic()
progressr::with_progress({
  mbb_player_box <- hoopR::load_mbb_player_box(2006:2021)
})
tictoc::toc()
glue::glue("MBB player boxscore data from {length(unique(mbb_player_box$game_id))} games.")
...
shot_clock_range_df %>% 
  ...
  ggsave('figures/hoopR_chicklet.png', width = 5.0, height = 5.0, units=c("in"))